    ...
    carTotalPrice: (state, getters, rootState) => {
      if (rootState.products.productsList && rootState.products.productsList.merch_list) {
        return rootState.products.productsList.merch_list[0].list.reduce((previousValue, currentValue, index) => {
          previousValue = index === 1 ? (previousValue.selected === '1' ? parseFloat(previousValue.productprice) * parseInt(previousValue.total) : 0) : previousValue
          currentValue = currentValue = currentValue.selected === '1' ? parseFloat(currentValue.productprice) * parseInt(currentValue.total) : 0
