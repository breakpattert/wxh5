...
<div class="container">
  ...
  <div class="managementList">
        <!-- <div class="handerItem" @click="$router.push('myOrder')">
         ...
         <div class="handerItem" @click="$router.push({ path: '/pages/voucher/voucher'})">
            <div class="icon">
  ...
  methods: {
    async hasauth() {
      ...
      if( this.$store.state.user.userInfo.openId.slice(0,3)=='wap'){
           this.hasAuthInfo = true;
           this.avatarUrl = this.$store.state.user.userInfo.avatarUrl? this.$store.state.user.userInfo.avatarUrl:'https://sh.szwfy.com.cn/attachment/api/20180909/f03ffb8d55c752573b449a7c057c2731.png'
           this.userInfo = this.$store.state.user.userInfo
    ...
    goLogin_person(){
      if(this.hasAuthInfo){
          ...
          this.$router.push({ path: '/pages/login/loginIndex' })
      }
    },
     goAuth() {
