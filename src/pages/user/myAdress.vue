    ...
    <div class="myAress w100">      
        <div class="item lineBottom  ">
            <div class="reciveNamePhone ml15 mb20">
              <span class="name f16 mr21 color32">{{item.realname}}</span>
              <span class="phone f16 color32">{{item.mobile}}</span>
            </div>
            <div class="adress ml15 f16 color32">{{item.province}}{{item.city}}{{item.area}}{{item.address}}</div>
       </div>      
    </div>
...
export default {
  ...
  async created() {
    ...
    console.log(this.getAdressList);
  }
};
</script>
