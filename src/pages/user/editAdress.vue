...
<div class="container lineTop">
   ...
   <!-- <div class="phone w100 h40 lineBottom" @click="toMap">
    ...
    <div class="set w100 h40 lineBottom mb20">
      ...
      <switch class="fr mr15" v-if="data.isdefault=='1'" checked @change="switch1Change"/>
       <switch class="fr mr15" v-if="data.isdefault=='0'" @change="switch1Change"/>
    </div>
  ...
  methods: {
    ...
    async submit(){
        ...
        if(!errors){
          this.$http.addAdress(this.data.realname, this.data.mobile, this.data.province, this.data.city, this.data.area, this.data.address, this.data.isdefault, this.openid, this.authkey).then(function(res){
            if(!res.error){
              ...
              setTimeout(function(){
                that.$router.push({ path: "/pages/user/myAdress", isTab: false });
              },1000)
            }else{
     ...
     async editsub(){
      ...
      that.validate((errors, fields) => {
        if(!errors){
          this.$http.editAdress(this.$route.query.id,this.data.realname, this.data.mobile, this.data.province, this.data.city, this.data.area, this.data.address, this.data.isdefault).then(function(res){
            if(!res.error){
