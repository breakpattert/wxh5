<template>
  <div class="container goodsDetail">
     ...
     <mt-popup  v-model="showbottom"  position="bottom">
      ...
      <div class="readyToBuy" >
        ...
        <div class="choose_size">
          ...
          <div class="sizes">
          	<!---->
            <div class="size_2"  @click="colorActive(index,goodsDetail.id,item.id)"
            v-for="(item,index) in colorSize[0]?colorSize[0].items:[]" :key='index' :class="{'active_2':colorIsSelect == index}"
            >{{item.title}}
    ...
    <div class="shopping_mall" @click="$router.push({ path:'/pages/busineshop/shop',query:{id:goodsDetail.merchid}})">
      <div class="pull_left">
    ...
    <div class="discounts bgcfff lineBottom">
      ...
      <div class="check_all" @click="$router.push({ path:'/pages/goodsDetail/goodsDetailRate',query:{id:goodsDetail.id}})">
        查看全部>>
      </div>
    </div>
    <!-- 循环的数据 -->
    ...
    <div class="fixedBox">
      <div class="todos">
        ...
        <div class="three" @click="$router.push({ path:'/pages/user/myCar'})">
...
export default {
  ...
  methods: {
   ...
   async togglePopup() {
      ...
      if(this.optionid!=null){
         ...
         this.$store.commit("SET_CREATE_ORDER_INFO",{
           ...
           optionid:this.optionid
            ...
            this.showbottom =!this.showbottom
            this.$router.push({ path:'/pages/user/myPay',query:{from:'goodsDetail'}})
    ...
    async addCar(){
          ...
          if(this.optionid!=null){
          const data  = await this.$http.addGoods(
          this.$route.query.id, 
          this.currentNum,
          this.optionid,
          this.$store.state.user.userInfo.openId).then(res=>{
              if(res.error==0){
    ...
    async getPay_coupon(id){
        const  [err ,res]  = await this._to(this.$http.getPay_coupon(id,this.$store.state.user.userInfo.openId ))  
       if(res.error==0){
   ...
   async colorActive(index,id,colorId){
      ...
      if( this.colorId !=null&&this.sizeId!=null){
          const [err,res] = await this._to(this.$http.getStock(id,this.colorId,this.sizeId))
   ...
   async sizeActive(index,id,sizeId){
    ...
    add(stock){
        ...
        if(this.currentNum<=parseInt(stock)){
          ...
          this.$tips.toast(`最大添加数量${stock?stock:0}`,function () { },'none')
        }
    ...
    async like(isfavorite){
        ...
        const res = await  this.$http.goodstoggle(this.goodsDetail.id, 
        isfavorite?'0':'1',
        ...
        if(res.status=='Success'){
          const [err , res] = await this._to(this.$http.getGoodsDetail(this.$route.query.id,this.$store.state.user.userInfo.openId))
  ...
  //  const arr =[this.$store.state.user.userInfo.openId , this.$store.state.user.userInfo.authkey]
    ...
    const [err , res] = await this._to(this.$http.getGoodsDetail(this.$route.query.id,this.$store.state.user.userInfo?this.$store.state.user.userInfo.openId:null))
    this.goodsDetail=res.goods
    ...
    const [err1, res1 ] = await this._to(this.$http.get_picker(this.$route.query.id ))
    this.colorSize =res1.specs;
    console.log(this.colorSize,8);
    ...
    let getAdressList = await this.$http.getAdressList(this.openid, this.authkey)
  ...
  onPullDownRefresh() { }, 
    ...
    console.log(options.target.dataset.param)
    debugger
    return {
      title: app.globalData.userInfo.nickName + '天天逛街...',
      path: '/pages/goodsDetail/index' + options.target.dataset.param
    }
  }
  ...
  .fixedBox {
    ...
    > div.todos {
      ...
      > .three {
        ...
        .star {
          background: url(#{$imgUrl}/icon_kindheart.png) no-repeat center/cover;
          width:18.5px;
          height: 18px;
        ...
        .starActive{
          background: url(#{$imgUrl}/icon_kindheart_act.png) no-repeat center/cover; 
        }
        .shopcar {
